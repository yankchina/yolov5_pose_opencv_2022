...
config = {'person_conf_thres': 0.7, 'person_iou_thres': 0.45, 'kp_conf_thres': 0.5,
        ...
        'segments': {1: [5, 6], 2: [5, 11], 3: [11, 12], 4: [12, 6], 5: [5, 7], 6: [7, 9], 7: [6, 8], 8: [8, 10],9: [11, 13], 10: [13, 15], 11: [12, 14], 12: [14, 16]},
...
class yolov5():
    def __init__(self, modelpath):
        ...
        coco_class_file_path = os.path.join(self.app_path, 'data', 'coco.names')
        crowd_class_file_path = os.path.join(self.app_path, 'data', 'crowd.names')
        if modelpath.endswith('_coco.onnx'):
            with open(coco_class_file_path, 'rt') as f:
                self.classes = f.read().rstrip('\n').split('\n')
                self.lines = config['segments']
                self.kp_face = config['kp_face']
        else:
            with open(crowd_class_file_path, 'rt') as f:
                self.classes = f.read().rstrip('\n').split('\n')
